...
<template>
  <router-link
    :to="`/book/${book.id}`"
    class="card card-hover block no-underline relative overflow-hidden group"
  >
    <!-- Cover gradient banner -->
    <div
      class="h-16 relative overflow-hidden"
    ...
    >
      ...
      />
      <div class="absolute inset-0" style="background: linear-gradient(to bottom, transparent 40%, rgba(0,0,0,0.3) 100%)" />
      <div class="absolute top-2.5 left-3 right-3 flex items-start justify-between gap-2">
        <div class="flex items-center gap-1.5">
          <CategoryBadge :category="book.category" />
          <span
            v-if="downloaded"
            class="flex items-center justify-center w-5 h-5 rounded-full bg-emerald-500/90 text-white shrink-0"
            title="Скачано"
          >
            <IconCheck :size="12" />
          </span>
    ...
    <div class="p-4 pt-3.5">
      <h3 class="text-[14px] font-semibold mb-1 line-clamp-2 leading-snug text-[--t1] group-hover:text-white transition-colors" :title="book.title">
        {{ book.title }}
      </h3>
      <p class="text-[12px] mb-3 line-clamp-1 text-[--t2]" :title="book.author">
        {{ book.author }}
      ...
      <div class="flex items-center justify-between mt-auto">
        <div v-if="book.tags.length" class="flex gap-1 flex-1 min-w-0 overflow-hidden">
          <span
            v-for="tag in book.tags.slice(0, 2)"
            :key="tag"
            class="text-[10px] px-2 py-0.5 rounded-full text-[--t3] font-medium flex-shrink-0 truncate max-w-[80px]"
            style="background: rgba(255,255,255,0.05)"
          >
            {{ tag }}
          </span>
          <span v-if="book.tags.length > 2" class="text-[10px] text-[--t3] flex-shrink-0">
            +{{ book.tags.length - 2 }}
          </span>
        </div>

        <div v-if="book.rating" class="flex gap-0.5 text-amber-400/70 flex-shrink-0 ml-2">
