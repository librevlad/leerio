...
const coverPattern: Record<string, string> = {
  'Бизнес':         'radial-gradient(circle at 80% 20%, rgba(255,255,255,0.18) 0%, transparent 50%), radial-gradient(circle at 20% 80%, rgba(0,0,0,0.15) 0%, transparent 50%)',
  'Отношения':      'radial-gradient(circle at 20% 30%, rgba(255,255,255,0.15) 0%, transparent 50%), radial-gradient(circle at 80% 70%, rgba(0,0,0,0.15) 0%, transparent 50%)',
  'Саморазвитие':   'radial-gradient(circle at 70% 20%, rgba(255,255,255,0.15) 0%, transparent 50%), radial-gradient(circle at 30% 80%, rgba(0,0,0,0.15) 0%, transparent 50%)',
  'Художественная': 'radial-gradient(circle at 30% 30%, rgba(255,255,255,0.18) 0%, transparent 50%), radial-gradient(circle at 70% 70%, rgba(0,0,0,0.15) 0%, transparent 50%)',
  'Языки':          'radial-gradient(circle at 80% 30%, rgba(255,255,255,0.15) 0%, transparent 50%), radial-gradient(circle at 20% 70%, rgba(0,0,0,0.15) 0%, transparent 50%)',
}
...
<template>
  <div class="card overflow-hidden">
    ...
    >
      ...
      />
      <div class="absolute inset-0" style="background: linear-gradient(to bottom, transparent 30%, rgba(0,0,0,0.5) 100%)" />

      <!-- Badges on banner -->
      <div class="absolute top-4 left-5 right-5 flex items-start justify-between gap-2">
        <CategoryBadge :category="book.category" />
        <StatusBadge v-if="book.status" :status="book.status" />
      </div>

      <!-- Progress ring on banner (if progress > 0) -->
      <div v-if="book.progress > 0" class="absolute bottom-4 right-5">
        <ProgressRing :percent="book.progress" :size="56" :stroke="4" />
      </div>

      <!-- Title/author on banner bottom -->
      <div class="absolute bottom-4 left-5 right-24">
        <h1
          class="text-[22px] sm:text-[26px] font-bold tracking-tight leading-tight text-white drop-shadow-lg"
          :title="book.title"
        >
          {{ book.title }}
        </h1>
        <p class="text-[14px] mt-1 text-white/70 drop-shadow" :title="book.author">
          {{ book.author }}
    ...
    <!-- Metadata row -->
    <div v-if="hasMetadata()" class="px-5 py-4 flex flex-wrap items-center gap-5 border-t border-white/[0.04]">
      ...
      <div v-if="book.rating" class="flex items-center gap-2.5 ml-auto">
